...
class Tile:
    ...
    def Draw(self, screen, is_drawing_bridges):
        def draw_river():
            ...
            screen.blit(pygame.transform.scale(image, (self.Offset, self.Offset)),
                        (self.Position.X * self.Offset, self.Position.Y * self.Offset))
        ...
        if not self.Harbor and self.Traverseable and not self.Park and not self.River or self.Bridge:
            ...
            if self.Up != None and self.Up.Traverseable or (self.Bridge and (
                            self.Down != None and self.Down.Traverseable and self.Up != None and self.Up.Traverseable)):
                screen.blit(pygame.transform.scale(self.DefaultTexture, (_width + dim, _width + dim)),
                            (_width + self.Position.X * self.Offset,
                             self.Position.Y * self.Offset))
            if self.Down != None and self.Down.Traverseable or (self.Bridge and (
                            self.Down != None and self.Down.Traverseable and self.Up != None and self.Up.Traverseable)):
                screen.blit(pygame.transform.scale(self.DefaultTexture, (_width + dim, _width + dim)),
                            (_width + self.Position.X * self.Offset,
                             _width * 2 + self.Position.Y * self.Offset))
            if self.Left != None and self.Left.Traverseable or (self.Bridge and (
                            self.Left != None and self.Left.Traverseable and self.Right != None and self.Right.Traverseable)):
                screen.blit(pygame.transform.scale(self.DefaultTexture, (_width + dim, _width + dim)),
                            (self.Position.X * self.Offset,
                             _width + self.Position.Y * self.Offset))
            if self.Right != None and self.Right.Traverseable or (self.Bridge and (
                            self.Left != None and self.Left.Traverseable and self.Right != None and self.Right.Traverseable)):
                screen.blit(pygame.transform.scale(self.DefaultTexture, (_width + dim, _width + dim)),
    ...
    while (node != None):
        ...
        if (column > 0):
            ...
            def UpdateNode(node):
                ...
                node.DefaultTexture = pygame.image.load(os.path.join("Content", "white_pixel.png")).convert()
            ...
            if row == 0:
                ...
                above_line = node.Up
                if random.uniform(0,
                                  1) > 0.3 and above_line.Left != None and not above_line.Left.River and prev_node.River:
                    UpdateNode(node)
    ...
    while (node != None):
        ...
        if node.River:
            if (column > 0 and row > 0 and column < dimension - 1 and row < dimension - 1):
                if (
                            not node.Left.River and not node.Right.River and node.Left.Traverseable and node.Right.Traverseable) or (
                            not node.Down.River and not node.Up.River and node.Up.Traverseable and node.Down.Traverseable):
                    if (
                                not node.Left.River and not node.Right.River and node.Left.Traverseable and node.Right.Traverseable):
                ...
                if (
                            node.Left.Traverseable and node.Right.Traverseable and not node.Left.River and not node.Right.River):
                    node.Left.IsEntryOfBridge = True
                    node.Right.IsEntryOfBridge = True
                    UpdateNode(node)
    ...
    if tiles.IsEmpty:
        ...
        if node.Position != (0, 0):
            if (random.uniform(0, 1)) > 0.8 and node.River and not node.Bridge:
                ...
                node.DefaultTexture = pygame.image.load(os.path.join("Content", "harbor.png")).convert()
                node.UpdateProps(properties)
            if (random.uniform(0, 1)) > 0.8 and not node.River and not node.IsEntryOfBridge:
                properties = Node(NotTraverseable, Empty)
                node.DefaultTexture = pygame.image.load(os.path.join("Content", "house.png")).convert()
                node.UpdateProps(properties)
            elif not node.IsEntryOfBridge and (random.uniform(0, 1)) > 0.85 and not node.River and (
                        (node.Left != None and node.Left.Traverseable and not node.Left.River) or (
                                node.Right != None and node.Right.Traverseable and not node.Right.River) or (
                            node.Up != None and node.Up.Traverseable and not node.Up.River) or (
                        node.Down != None and node.Down.Traverseable and not node.Down.River)):
                properties = Node(Park, Empty)
                node.DefaultTexture = pygame.image.load(os.path.join("Content", "park.png")).convert()
                node.UpdateProps(properties)
    ...
    def build_board():
        ...
        for row in range(dimension):
            for column in range(dimension):
                node = Tile(Vector2(column, row), os.path.join("Content", "white_pixel.png"), offset, Empty)
