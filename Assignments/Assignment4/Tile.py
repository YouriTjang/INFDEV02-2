        ...
        for column in range(dimension):
            if (random.uniform(0, 1) > 0.75) and row > 0 and column > 0:
                if (random.uniform(0, 1) > 0.75):
                    ...
                    node = Tile(Point(column, row), os.path.join("Content", "park.png"), offset, properties)
                else:
                    properties = Node(NotTraverseable, Empty)
                    node = Tile(Point(column, row), os.path.join("Content", "house.png"), offset, properties)
            else:
                node = Tile(Point(column, row), os.path.join("Content", "white_pixel.png"), offset, Empty)
