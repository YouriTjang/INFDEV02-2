...
\begin{slide}{
\item Review assignment 2A
\item Today we will introduce linked lists
}\end{slide}


\SlideSection{Problem discussion}
\SlideSubSection{Review: Objects and lists}
\begin{slide}{
\item All players
...
\SlideSubSection{Lists functionality}
\begin{slide}{
\item Add elements to the list (append(3))
\item read (pop() or list[2])
\item change elements (list[2] = 3)
\item remove elements from the list (remove(3))
\item \textbf{C}reate, \textbf{R}ead, \textbf{U}pdate and \textbf{D}elete
}
\end{slide}
...
\begin{slide}{
\item In order to explore the inner workings of data structures
\item we will build our own list-data structure
\item along the way we will learn a lot about:
\begin{itemize}
\item references
\item iteration
\item working with classes and objects
\end{itemize}
}\end{slide}
...
\item so the \textit{for ... in range(...)} won't work
\item we need a variable to indicate if a list is empty
}\end{slide}

\begin{frame}[fragile]{isEmpty}
\begin{lstlisting}
